...
export default function index() {
    ...
    async data => {
      let sendData =
        apiData.type == 1 || apiData?.type == 3
          ? apiData.decimal_allow == '2' ? data.deposit_rate : String(parseFloat(data.deposit_rate).toFixed(2))
  ...
  const rangeValue = React.useMemo(() => {
    ...
    if (apiData.type == 1) {
      if (apiData?.decimal_allow == '2') {
        return [parseInt(apiData.min_rate), parseInt(apiData.max_rate)]
      } else {
    ...
    } else if (apiData.type == 3 && apiData.set_range == 1) {
      return [parseFloat(apiData.rate_detail.sub_type_data[pickerValue].rate[0].min_rate), parseFloat(apiData.rate_detail.sub_type_data[pickerValue].rate[0].max_rate)]
    } else {
      return [5, 15, 5, 15];
    }
  }, [apiData, pickerValue]);
  ...
  const InputValue = value => {
    if (isNaN(parseFloat(value) || _.isEmpty(value))) {
      return '';
    }
    if (apiData?.type == 1 || apiData?.type == 3) {
      ...
      if (apiData?.decimal_allow == '2') {
        return String(apiData?.rate[value].toFixed(0));
      } else {
              ...
              <View style={styles.ComulativeRatePresentContainer}>
                ...
                <TextMedium color={COLORS.PRIMARY}>
                  {apiData?.deposit_rate == 'N/A'
                    ...
                    : `${apiData?.decimal_allow == '2' ? parseFloat(apiData?.deposit_rate).toFixed(0) : parseFloat(apiData?.deposit_rate).toFixed(2)}%`}
                </TextMedium>
              </View>
            </View>
          </Container>
                ...
                <Controller
                  ...
                  render={({ field: { onChange, value }, fieldState: { error } }) => {
                    return (
                      <>
                        <View>
                          {apiData.type == 3 && apiData.set_range == 1 ?
                            apiData.rate_detail.sub_type_data.map((item, index) => {
                              return index == pickerValue && <Slider
                                style={{
                                  width: '70%',
                                  height: 40,
                                  alignSelf: 'center',
                                ...
                                minimumValue={item.decimal_allow == '2' ? parseInt(item.rate[0].min_rate) : parseFloat(item.rate[0].min_rate)}
                                maximumValue={item.decimal_allow == '2' ? parseInt(item.rate[0].max_rate) : parseFloat(item.rate[0].max_rate)}
                                onSlidingComplete={() =>
                              ...
                              step={
                                apiData?.type == 1 || apiData?.type == 3
                                  ? apiData?.decimal_allow == '2' ? 1 : 0.01
                                  : 1
                              }
                              ...
                              value={
                                inputRef.current !== null
                                  ? inputRef.current.isFocused()
                                    ? InputValue(value) * 1
                                    : apiData?.decimal_allow == '2' ? parseInt(value) : value
                                  : value
                              }
                              onValueChange={v => {
                                trigger();
                          ...
                          <TextMedium
                            ...
                            size={FONT_SIZE.BODY_2}>
                            {`${apiData?.decimal_allow == '2' ? parseInt(rangeValue[0]) : parseFloat(rangeValue[0]).toFixed(2)}%`}
                          </TextMedium>
                          <TextMedium
                            style={{
                              position: 'absolute',
                            ...
                            size={FONT_SIZE.BODY_2}>
                            {`${apiData?.decimal_allow == '2' ? parseInt(rangeValue[1]) : parseFloat(rangeValue[1]).toFixed(2)}%`}
                          </TextMedium>
                        </View>
                        <View style={styles.CumulativeRateNew}>
                          ...
                          <Input
                            ...
                            onChangeText={v => {
                              const filteredValue = apiData?.decimal_allow == '2'
                                ? v.replace(/\.\d*/, '')
                                : v;
      ...
      return (
        <TextRegular style={{ marginTop: ViewScale(10) }} size={FONT_SIZE.BODY_2}>
          <TextMedium color={COLORS.PRIMARY} size={FONT_SIZE.BODY_2}>
            {apiData.min_rate
              ? String(parseFloat(apiData.min_rate).toFixed(apiData?.decimal_allow == '2' ? 0 : 2))
              : apiData.min_rate}
            %
          </TextMedium>
          ...
          <TextMedium color={COLORS.PRIMARY} size={FONT_SIZE.BODY_2}>
            {apiData.max_rate
              ? String(parseFloat(apiData.max_rate).toFixed(apiData?.decimal_allow == '2' ? 0 : 2))
              : apiData.max_rate}
            %
          </TextMedium>
        </TextRegular>
